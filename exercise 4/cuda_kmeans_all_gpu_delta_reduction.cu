...
void find_nearest_cluster(int numCoords,
  ...
  for (i = tid1; i < numCoords * numClusters; i += blockDim.x) {
...
void update_centroids(int numCoords,
                      ...
                      double *deviceClusters)    //  [numCoords][numClusters])
{
   // edw exoume sto shared memory to deviceClusters
  /* TODO: additional steps for calculating new centroids in GPU? */
  int tid = get_tid();
  int clustersize_id = tid % numClusters;
  if(tid < numClusters*numCoords) {
     if(devicenewClusterSize[clustersize_id] > 0) { //mporei kapoia clusters na einai 0 prin gemisoun
       deviceClusters[tid] = devicenewClusters[tid]/devicenewClusterSize[clustersize_id];
     }
     devicenewClusters[tid]=0; //ananewsh kai sta dyo wste na ksanagemisoun me freska apotelesmata
   }
  __syncthreads();
  devicenewClusterSize[clustersize_id]=0;
}
...
void kmeans_gpu(double *objects,      /* in: [numObjs][numCoords] */
  ...
  do {
    ...
    /* TODO: change invocation if extra parameters needed, edw kaneis thn pragmatikh douleia kai allazeis to membership klpklp kai ftiaxneis to new clusters*/
    ...
    /* TODO: can use different blocksize here if deemed better, i think one thread for every coordinate of each cluster(numCoords*numClusters), ara ena thread gia kathe element(koutaki) sto clusters*/
    ...
    /* TODO: calculate dim for "update_centroids"(me tin update cendroids kaneis mono to copy apo clusters, clusterSize sta newClusters,newClusterSize)*/
    //sth shared mnhmh ta exeis apothikevmena ws [numCoords][numClusters], ara to prwto thread[0][0], to deytero thread[0][1], to trito [0][2] klpklp, oxi san prin pouu kathe thread phdage seira 
...
//  fprintf(fp, "%s,%d,%lf,%lf,%lf\n", "All_GPU_delta_reduction", blockSize, timing / loop, timer_min, timer_max);
  fprintf(fp, "%s,%d,%lf,%lf,%lf,%lf,%lf,%lf\n", "ALL-GPU-Delta", blockSize, timing / loop, timer_min, timer_max, 1000 * cpu_time / loop,1000 * gpu_time / loop,1000 * transfers_time / loop);
