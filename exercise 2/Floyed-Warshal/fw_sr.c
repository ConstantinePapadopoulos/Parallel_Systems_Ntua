/*ursive implementation of the Floyd-Warshall algorithm.
	...
	if(myN<=bsize)
		...
		#pragma omp parallel
    	{
        	#pragma omp single
        	{
				...
				// #pragma omp task
				// FW_SR(A,arow, acol+myN/2,B,brow, bcol,C,crow, ccol+myN/2, myN/2, bsize);//paralel tasks
				// #pragma omp task
				// FW_SR(A,arow+myN/2, acol,B,brow+myN/2, bcol,C,crow, ccol, myN/2, bsize);// paralel tasks
				// #pragma omp taskwait
				...
				{
					#pragma omp task
					FW_SR(A,arow+myN/2, acol,B,brow+myN/2, bcol+myN/2,C,crow+myN/2, ccol, myN/2, bsize);//parallel tasks
					FW_SR(A,arow, acol+myN/2,B,brow, bcol+myN/2,C,crow+myN/2, ccol+myN/2, myN/2, bsize);//paralel tasks
					#pragma omp taskwait
