...
int main(int argc, char ** argv) {
    ...
    if (argc!=5) {
        ...
        global[0]=atoi(argv[1]);		//X	
        global[1]=atoi(argv[2]);		//Y
        grid[0]=atoi(argv[3]);			//Posa processes sto X
        grid[1]=atoi(argv[4]);			//posa processes sto Y
    }
...
//third try	
   MPI_Scatterv(
    	(rank == 0) ? U[0] : NULL,			//mono gia rank 0 alliws idk 8a to dw meta
...
//cane to initialize
	...
	/*Fill your code here*/
	//edw ta pragmata dyskolevoun 8elw 2 datatypes, gia row kai gia column 
    ...
    MPI_Cart_shift(CART_COMM, 1, 1, &west, &east);		// 1 -> axonas x +-1 neighbour	
							//an den exei tote ginetai MPI_PROC_NULL
	...
	i_min = 1; i_max = valid_i;		//ola einai local[0] & [1] alliws de xerw isws einai swsto isws oxi logika einai alla elpizw na mi xreiastei na ma8w
	j_min = 1; j_max = valid_j;
	...
	*/
	...
	MPI_Sendrecv(&u_previous[local[0]][1], 1, row_type, south, 0,
             ...
             CART_COMM, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&u_previous[1][local[1]], 1, col_type, east, 0,
             &u_previous[1][0], 1, col_type, west, 0,
             CART_COMM, MPI_STATUS_IGNORE);
	MPI_Sendrecv(&u_previous[1][1], 1, col_type, west, 0,
             &u_previous[1][local[1]+1], 1, col_type, east, 0,
             CART_COMM, MPI_STATUS_IGNORE);
