...
/*genika xreiazomaste 2 rounds gia to computation giati o pinakas ananewnetai se 2 faseis, th red kai th black fash, sth red fash allazei o ftiaxnw ta red spots((i+j)%2==0) ta opoia epeidh den einai adjacent mporoun na ypologistoyn parallila, kai oi allages aytes prepei na einai orates stis geitonikes mou diergasies gia th black fash, epomenws exw to prwto round tou sendrecv gia na parw plirofories apo geitonikes diergasies pou aforoyn sth prohgoumenh xronikh stigmh, kanw red sor, kai meta enw allon ena guro sendrecv gia na dw tis allages poy ekanan oi geitones sth red fash stous pinakes tous, pou tha xreiastw egw ston diko mou upologismo sth black fash*/
	//here is the red phase
        // HERE I SEND MY ENTIRE GHOST CELLS NOT JUST THE RED OR BLACK PARTS!!!

// Send our first interior row and receive the ghost row from north.
MPI_Sendrecv(
    &u_previous[1][1],       // Start sending from row 1, col 1 (interior data)
    local[1],                // Number of doubles (width of interior row)
    MPI_DOUBLE,              // Data type
    north,                   // Destination: north neighbor
    0,                       // Send tag

    &u_previous[0][1],       // Receive into ghost row at row 0, col 1
    local[1],                // Number of doubles expected
    MPI_DOUBLE,              // Data type
    north,                   // Source: north neighbor
    0,                       // Receive tag
