...
//    if (rank == 0) printf("dataset_size = %.2f MB    numObjs = %ld    numCoords = %ld    numClusters = %ld\n", dataset_size, numObjs, numCoords, numClusters);
    ...
    */
    ...
    if (rank == 0) {
         ...
         *       displs: displacement of each rank's data, displacement:starting index in the tot_membership array where the data from each rank will be placed.

         */
	for(i=0; i<size; i++){
		recvcounts[i]=rank_numObjs;
	...
	int MPI_Gatherv(
    		...
    		int *recvcounts,        // Array of number of elements received from each process
    		int *displs,            // Array of displacements in receive buffer
    		MPI_Datatype recvtype,  // Data type of the receive buffer
